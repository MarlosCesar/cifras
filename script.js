...
let appState = {
  ...
  darkMode: false // estado do modo noturno
};
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  renderTabs();
  renderCifras();
  applyDarkMode(); // Aplica o modo noturno salvo
});
...
function loadState() {
  try {
    ...
    if (savedState) {
      ...
      appState.darkMode = savedState.darkMode === true; // Carrega o modo noturno
...
function updateSelectionControls() {
  ...
  controls.style.display = selectedCount > 0 ? '' : 'none';
  selectAllBtn.style.display = (selectedCount === totalCount || totalCount === 0) ? 'none' : '';
}

// ================ FULLSCREEN API ================
// The openFullscreenView function now uses the Fullscreen API so that the overlay occupies the entire screen.
function openFullscreenView(imageSrc, imageName) {
  ...
  function updateTransform() {
    // Set the transform origin based on calculated percentages
    img.style.transformOrigin = `${originX * 100}% ${originY * 100}%`;
    img.style.transform = `scale(${scale}) translate(${translateX / scale}px, ${translateY / scale}px)`;
  }

  // Request fullscreen for the overlay so that it covers the entire screen (including browser chrome where possible)
  if (overlay.requestFullscreen) overlay.requestFullscreen();
  else if (overlay.webkitRequestFullscreen) overlay.webkitRequestFullscreen();

  // Mouse: zoom (with scroll) and drag
  ...
  // Mobile: pinch/drag (prevent closing during pinch)
  let lastTapTime = 0, tapTimeout = null;
  ...
  // Close fullscreen on double click/double tap on the overlay (not on the image)
  ...
  // Listen for fullscreen change to close overlay if user exits fullscreen natively
